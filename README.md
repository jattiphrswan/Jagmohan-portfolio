...
- **Direct Contact Workflow**: Enquiries submitted via the public contact form are validated server-side, rate-limited, protected with honeypot fields, and routed directly to the owner's inbox via Resend HTTPS API with `Reply-To` set to the visitor's email. No contact messages or leads are persisted in the database.
...
- **Email Delivery**: Resend Node SDK (`resend` v6) via HTTPS API (eliminates SMTP blocks and OAuth complexity)
...
# Resend HTTPS API Contact Form Configuration (Post-N15 Hotfix)
RESEND_API_KEY=
CONTACT_TO_EMAIL=

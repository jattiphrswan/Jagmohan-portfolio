...
export default function AdminProjectsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`${API_BASE}/api/admin/projects/${deleteTarget.id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      ...
      setActionSuccess(`Project "${deleteTarget.title}" deleted successfully.`);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 mb-1">
            ...
            <span>/</span>
            <span className="text-slate-800 font-semibold">Projects Management</span>
          </div>
      ...
      <SectionCard>
        {loading ? (
          ...
          <div className="py-12 text-center text-xs text-slate-500">
            ...
            <p className="mt-1 text-slate-400">Click "Add New Project" to create your first portfolio entry.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 font-semibold">
                  <th className="pb-3 pr-4 font-semibold">Project Title</th>
                ...
                {projects.map((project) => (
                  <tr key={project.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 pr-4">
                      <div className="font-bold text-slate-900 leading-snug">{project.title}</div>
                      <div className="text-[11px] text-slate-400 font-mono mt-0.5">/{project.slug}</div>
                    </td>
        ...
        >
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="mt-2 text-xs text-slate-600 leading-relaxed">
              Are you sure you want to delete <strong className="text-slate-900 font-semibold">"{deleteTarget.title}"</strong>? This will permanently remove the project from both the admin dashboard and the public portfolio.
            </p>
